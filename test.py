...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    conf_count = 0
    conf_thr = 0.8  # threshold for confidence-based metrics
    conf_selected = 0  # how many preds have conf >= conf_thr
    conf_correct = 0  # how many of those are correct
    length_of_data = 0
    infer_time = 0
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        for gt, pred, pred_max_prob in zip(labels, preds_str, preds_max_prob):
            ...
            if len(gt) > 0:
                char_edit_total += edit_distance(pred, gt)
                char_count_total += len(gt)
            ...
            conf_sum += float(confidence_score)
            conf_count += 1
            if float(confidence_score) >= conf_thr:
                conf_selected += 1
                if pred == gt:
                    conf_correct += 1
...
def test(opt):
    ...
    if isinstance(state, dict):
        try:
            ...
            # Try stripping a possible 'module.' prefix from DataParallel checkpoints.
            if any(isinstance(k, str) and k.startswith('module.') for k in state.keys()):
                ...
                try:
                    ...
                    loaded = True
                    print('Info: stripped "module." prefixes while loading checkpoint.')
                except RuntimeError as e2:
                    model.load_state_dict(stripped, strict=False)
                    loaded = True
                    print('Warning: partially loaded checkpoint after stripping "module.":', e2)
            if not loaded:
