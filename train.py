...
def train(opt):
    ...
    # weight initialization (HANYA kalau tidak pakai pretrained)
    if opt.saved_model == '':
        for name, param in model.named_parameters():
            if 'localization_fc2' in name:
                print(f'Skip {name} as it is already initialized')
    ...
    if opt.saved_model != '':
        ...
        #hapus prefix "module." kalau checkpoint dari DataParallel
    ...
    print("Some missing keys:", msg.missing_keys[:30])
    print("Some unexpected keys:", msg.unexpected_keys[:30])
    print("=" * 50)

    
    # Freeze parts of the network before wrapping with DataParallel / optimizer.
    # Adjust the prefixes as needed. Current default freezes TPS and the backbone.
    for name, p in model.named_parameters():
        if name.startswith('Transformation') or name.startswith('FeatureExtraction'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_iter', type=int, default=1400, help='number of iterations to train for')
    ...
    parser.add_argument('--lr', type=float, default=1.0, help='learning rate, default=1.0 for Adadelta')
